...
/**
 * Fetches and filters blog posts with common setup
 * @param projectId - Optional project ID to filter posts by
 * @returns Posts data with week info and pagination setup
 */
export async function getPosts(projectId?: string): Promise<PostsResult> {
  ...
  // Detect content indicators for each post
  const postsWithIndicators = new Map<string, ContentIndicators>();
  for (const post of posts) {
    postsWithIndicators.set(post.slug, detectContentIndicators(post.body));
  }
